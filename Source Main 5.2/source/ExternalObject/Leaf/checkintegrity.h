...
namespace leaf {
    ...
    bool GenerateCheckSum(const std::wstring& in_filename, OUT LPCHECKSUMTABLE lpCheckSumTable);
    bool GenerateCheckSum(IN const void* pBuffer, size_t size, const std::wstring& out_filename);
    bool GenerateCheckSum(const std::wstring& in_filename, const std::wstring& out_filename);

    class CCheckSumGenerator
    {
        struct SEGMENTINFO {
            std::wstring name;	//. file name or data name
    ...
    public:
        ...
        bool AddSegmentFromBuffer(const std::wstring& dataname, const void* pBuffer, size_t size);
        bool AddSegmentFromFile(const std::wstring& filename);
        bool RemoveSegment(const std::wstring& name);
        bool RemoveSegment(int index);
        void RemoveAllSegment();

        int GetSegmentCount() const;
        const SEGMENTINFO* GetSegmentInfo(const std::wstring& name);
        const SEGMENTINFO* GetSegmentInfo(int index);

        /* Generate 1KB CheckSum data */
        bool Generate(OUT LPCHECKSUMTABLE lpCheckSumTable);
        bool Generate(const std::wstring& out_filename);

        static bool GenerateCheckSumTable(IN const void* pBuffer, size_t size, OUT LPCHECKSUMTABLE lpCheckSumTable);
        static bool GenerateCheckSumTable(const std::wstring& in_filename, OUT LPCHECKSUMTABLE lpCheckSumTable);
        static bool GenerateCheckSumFile(IN const void* pBuffer, size_t size, const std::wstring& out_filename);
        static bool GenerateCheckSumFile(const std::wstring& in_filename, const std::wstring& out_filename);
    };

    /* Cyclic Redundancy Check (CRC) */

    bool GenerateCrc32Code(const void* pBuffer, size_t size, DWORD& dwCrc32);	/* Generate 32bit CRC code from buffer */
    bool GenerateCrc32Code(const std::wstring& filename, DWORD& dwCrc32);		/* Generate 32bit CRC code from file */
    ...
    public:
        ...
        bool GenerateCrc32Code(const std::wstring& filename, DWORD& dwCrc32);
