...
xfstreambuf::xfstreambuf(const std::wstring& filename)
{
    load(filename);
}
xfstreambuf::~xfstreambuf()
...
bool xfstreambuf::load(const std::wstring& filename)
{
    FILE* fp = NULL;
    fp = _wfopen(filename.data(), L"rb");
    if (fp) {
        fseek(fp, 0, SEEK_END);
...
bool xfstreambuf::save(const std::wstring& filename)
{
    FILE* fp = NULL;
    fp = _wfopen(filename.data(), L"wb");
    if (fp) {
