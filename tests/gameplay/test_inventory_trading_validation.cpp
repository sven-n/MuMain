          ...
          "[inventory][drag-drop][storage-type][uniqueness][6-2-2]")
{
    ...
    const int kOsbourne = static_cast<int>(STORAGE_TYPE::OSBOURNE_MIX);          // 7
    const int kJerridon = static_cast<int>(STORAGE_TYPE::JERRIDON_MIX);          // 8
    const int kChaosCard = static_cast<int>(STORAGE_TYPE::CHAOS_CARD_MIX);       // 9
    const int kCherryBlossom = static_cast<int>(STORAGE_TYPE::CHERRYBLOSSOM_MIX); // 10
    const int kExtractSeed = static_cast<int>(STORAGE_TYPE::EXTRACT_SEED_MIX);   // 11
    const int kSeedSphere = static_cast<int>(STORAGE_TYPE::SEED_SPHERE_MIX);     // 12
    const int kAttachSocket = static_cast<int>(STORAGE_TYPE::ATTACH_SOCKET_MIX); // 13
    const int kDetachSocket = static_cast<int>(STORAGE_TYPE::DETACH_SOCKET_MIX); // 14
    const int kLuckyItemTrade = static_cast<int>(STORAGE_TYPE::LUCKYITEM_TRADE); // 15
    const int kLuckyItemRefinery = static_cast<int>(STORAGE_TYPE::LUCKYITEM_REFINERY); // 16
    ...
    SECTION("All 18 STORAGE_TYPE values are pairwise distinct")
    {
        ...
        REQUIRE(kUndefined != kOsbourne);
        REQUIRE(kUndefined != kJerridon);
        REQUIRE(kUndefined != kChaosCard);
        REQUIRE(kUndefined != kCherryBlossom);
        REQUIRE(kUndefined != kExtractSeed);
        REQUIRE(kUndefined != kSeedSphere);
        REQUIRE(kUndefined != kAttachSocket);
        REQUIRE(kUndefined != kDetachSocket);
        REQUIRE(kUndefined != kLuckyItemTrade);
        REQUIRE(kUndefined != kLuckyItemRefinery);
        ...
        REQUIRE(kInventory != kOsbourne);
        REQUIRE(kInventory != kJerridon);
        REQUIRE(kInventory != kChaosCard);
        REQUIRE(kInventory != kCherryBlossom);
        REQUIRE(kInventory != kExtractSeed);
        REQUIRE(kInventory != kSeedSphere);
        REQUIRE(kInventory != kAttachSocket);
        REQUIRE(kInventory != kDetachSocket);
        REQUIRE(kInventory != kLuckyItemTrade);
        REQUIRE(kInventory != kLuckyItemRefinery);
        ...
        REQUIRE(kTrade != kOsbourne);
        REQUIRE(kTrade != kJerridon);
        REQUIRE(kTrade != kChaosCard);
        REQUIRE(kTrade != kCherryBlossom);
        REQUIRE(kTrade != kExtractSeed);
        REQUIRE(kTrade != kSeedSphere);
        REQUIRE(kTrade != kAttachSocket);
        REQUIRE(kTrade != kDetachSocket);
        REQUIRE(kTrade != kLuckyItemTrade);
        REQUIRE(kTrade != kLuckyItemRefinery);

        REQUIRE(kVault != kChaos);
        REQUIRE(kVault != kMyShop);
        REQUIRE(kVault != kTrainer);
        REQUIRE(kVault != kElpis);
        REQUIRE(kVault != kOsbourne);
        REQUIRE(kVault != kJerridon);
        REQUIRE(kVault != kChaosCard);
        REQUIRE(kVault != kCherryBlossom);
        REQUIRE(kVault != kExtractSeed);
        REQUIRE(kVault != kSeedSphere);
        REQUIRE(kVault != kAttachSocket);
        REQUIRE(kVault != kDetachSocket);
        REQUIRE(kVault != kLuckyItemTrade);
        REQUIRE(kVault != kLuckyItemRefinery);

        REQUIRE(kChaos != kMyShop);
        REQUIRE(kChaos != kTrainer);
        REQUIRE(kChaos != kElpis);
        REQUIRE(kChaos != kOsbourne);
        REQUIRE(kChaos != kJerridon);
        REQUIRE(kChaos != kChaosCard);
        REQUIRE(kChaos != kCherryBlossom);
        REQUIRE(kChaos != kExtractSeed);
        REQUIRE(kChaos != kSeedSphere);
        REQUIRE(kChaos != kAttachSocket);
        REQUIRE(kChaos != kDetachSocket);
        REQUIRE(kChaos != kLuckyItemTrade);
        REQUIRE(kChaos != kLuckyItemRefinery);

        REQUIRE(kMyShop != kTrainer);
        REQUIRE(kMyShop != kElpis);
        REQUIRE(kMyShop != kOsbourne);
        REQUIRE(kMyShop != kJerridon);
        REQUIRE(kMyShop != kChaosCard);
        REQUIRE(kMyShop != kCherryBlossom);
        REQUIRE(kMyShop != kExtractSeed);
        REQUIRE(kMyShop != kSeedSphere);
        REQUIRE(kMyShop != kAttachSocket);
        REQUIRE(kMyShop != kDetachSocket);
        REQUIRE(kMyShop != kLuckyItemTrade);
        REQUIRE(kMyShop != kLuckyItemRefinery);

        REQUIRE(kTrainer != kElpis);
        REQUIRE(kTrainer != kOsbourne);
        REQUIRE(kTrainer != kJerridon);
        REQUIRE(kTrainer != kChaosCard);
        REQUIRE(kTrainer != kCherryBlossom);
        REQUIRE(kTrainer != kExtractSeed);
        REQUIRE(kTrainer != kSeedSphere);
        REQUIRE(kTrainer != kAttachSocket);
        REQUIRE(kTrainer != kDetachSocket);
        REQUIRE(kTrainer != kLuckyItemTrade);
        REQUIRE(kTrainer != kLuckyItemRefinery);

        REQUIRE(kElpis != kOsbourne);
        REQUIRE(kElpis != kJerridon);
        REQUIRE(kElpis != kChaosCard);
        REQUIRE(kElpis != kCherryBlossom);
        REQUIRE(kElpis != kExtractSeed);
        REQUIRE(kElpis != kSeedSphere);
        REQUIRE(kElpis != kAttachSocket);
        REQUIRE(kElpis != kDetachSocket);
        REQUIRE(kElpis != kLuckyItemTrade);
        REQUIRE(kElpis != kLuckyItemRefinery);

        REQUIRE(kOsbourne != kJerridon);
        REQUIRE(kOsbourne != kChaosCard);
        REQUIRE(kOsbourne != kCherryBlossom);
        REQUIRE(kOsbourne != kExtractSeed);
        REQUIRE(kOsbourne != kSeedSphere);
        REQUIRE(kOsbourne != kAttachSocket);
        REQUIRE(kOsbourne != kDetachSocket);
        REQUIRE(kOsbourne != kLuckyItemTrade);
        REQUIRE(kOsbourne != kLuckyItemRefinery);

        REQUIRE(kJerridon != kChaosCard);
        REQUIRE(kJerridon != kCherryBlossom);
        REQUIRE(kJerridon != kExtractSeed);
        REQUIRE(kJerridon != kSeedSphere);
        REQUIRE(kJerridon != kAttachSocket);
        REQUIRE(kJerridon != kDetachSocket);
        REQUIRE(kJerridon != kLuckyItemTrade);
        REQUIRE(kJerridon != kLuckyItemRefinery);

        REQUIRE(kChaosCard != kCherryBlossom);
        REQUIRE(kChaosCard != kExtractSeed);
        REQUIRE(kChaosCard != kSeedSphere);
        REQUIRE(kChaosCard != kAttachSocket);
        REQUIRE(kChaosCard != kDetachSocket);
        REQUIRE(kChaosCard != kLuckyItemTrade);
        REQUIRE(kChaosCard != kLuckyItemRefinery);

        REQUIRE(kCherryBlossom != kExtractSeed);
        REQUIRE(kCherryBlossom != kSeedSphere);
        REQUIRE(kCherryBlossom != kAttachSocket);
        REQUIRE(kCherryBlossom != kDetachSocket);
        REQUIRE(kCherryBlossom != kLuckyItemTrade);
        REQUIRE(kCherryBlossom != kLuckyItemRefinery);

        REQUIRE(kExtractSeed != kSeedSphere);
        REQUIRE(kExtractSeed != kAttachSocket);
        REQUIRE(kExtractSeed != kDetachSocket);
        REQUIRE(kExtractSeed != kLuckyItemTrade);
        REQUIRE(kExtractSeed != kLuckyItemRefinery);

        REQUIRE(kSeedSphere != kAttachSocket);
        REQUIRE(kSeedSphere != kDetachSocket);
        REQUIRE(kSeedSphere != kLuckyItemTrade);
        REQUIRE(kSeedSphere != kLuckyItemRefinery);

        REQUIRE(kAttachSocket != kDetachSocket);
        REQUIRE(kAttachSocket != kLuckyItemTrade);
        REQUIRE(kAttachSocket != kLuckyItemRefinery);

        REQUIRE(kDetachSocket != kLuckyItemTrade);
        REQUIRE(kDetachSocket != kLuckyItemRefinery);

        REQUIRE(kLuckyItemTrade != kLuckyItemRefinery);
    }
}
