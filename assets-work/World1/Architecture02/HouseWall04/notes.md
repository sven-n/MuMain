...
Identity and World1 placement: L-corner masonry wall module. 8 existing placements; exact positions, XYZ rotations, scale and tile coordinates in `original/placements.json`.

Geometry: 56 → 88 triangles. Both modular end planes and exact L-corner edges remain unchanged; four broad masonry face interiors are dressed. Footprint, origin, orientation, bone pivots and all original corners retained. New recess depths are 0.18 units for ladder wear, 0.4 for roof interiors and 0.65 for stone panels. Explicit modified pairs/perimeters are recorded in `validation/blender.json`. This is conservative architectural edge refinement, with the primary visual gain supplied by the shared material paintings.
...
Bind bounds before: `[[-54.81, -50.9, 0.28], [246.23, 249.87, 264.48]]`.
Bind bounds after: `[[-54.81, -50.9, 0.28], [246.23, 249.87, 264.48]]`.
Every original vertex position/bone survives final conversion within 0.000000000 units. 48 protected triangles retain positions, bone assignments and UVs. Maximum retained UV delta 0; normal component delta 0.000000000; normal direction delta 0.00000000 degrees. Original UV ranges: `[[-2.2147, 4.0189], [-0.8458, 3.8119]]`.

Rig: `['joint_01']`. Names, indices, order, parents and original one-action lock metadata unchanged; 1 action keys. Full local pose evidence: `[{'file': 'HouseWall04.smd', 'bone_samples': 1, 'frames': 1, 'max_position_component': 0, 'max_euler_component_radians': 0}, {'file': 'HouseWall04_a00.smd', 'bone_samples': 1, 'frames': 1, 'max_position_component': 0, 'max_euler_component_radians': 0}]`. All 1 hierarchical action matrices match with maximum component delta 0; every action-frame bound matches with maximum component delta 0. Per-frame matrices/bounds evidence is in `validation/matrices-and-posed-bounds.json`.
