...
Identity and World1 placement: Straight masonry wall with timber base and stone foot. 3 existing placements; exact positions, XYZ rotations, scale and tile coordinates in `original/placements.json`.

Geometry: 36 → 52 triangles. Both modular end planes remain original triangles; only front/back masonry panel interiors are dressed. Footprint, origin, orientation, bone pivots and all original corners retained. New recess depths are 0.18 units for ladder wear, 0.4 for roof interiors and 0.65 for stone panels. Explicit modified pairs/perimeters are recorded in `validation/blender.json`. This is conservative architectural edge refinement, with the primary visual gain supplied by the shared material paintings.

Material slots, unchanged order: tile_wood01.jpg, tile_wood02.jpg, tile_ston04.jpg.
...
Frozen dependency containers retained byte for byte: tile_wood02.OZJ, tile_ston04.OZJ.
...
Bind bounds before: `[[-100.06, -50.9, 0.28], [100.03, 49.55, 264.48]]`.
Bind bounds after: `[[-100.06, -50.9, 0.28], [100.03, 49.55, 264.48]]`.
Every original vertex position/bone survives final conversion within 0.000000000 units. 32 protected triangles retain positions, bone assignments and UVs. Maximum retained UV delta 0; normal component delta 0.000000000; normal direction delta 0.00000000 degrees. Original UV ranges: `[[-1.0325, 1.7305], [-1.1787, 1.0611]]`.

Rig: `['Object02']`. Names, indices, order, parents and original one-action lock metadata unchanged; 1 action keys. Full local pose evidence: `[{'file': 'HouseWall01.smd', 'bone_samples': 1, 'frames': 1, 'max_position_component': 0, 'max_euler_component_radians': 0}, {'file': 'HouseWall01_a00.smd', 'bone_samples': 1, 'frames': 1, 'max_position_component': 0, 'max_euler_component_radians': 0}]`. All 1 hierarchical action matrices match with maximum component delta 0; every action-frame bound matches with maximum component delta 0. Per-frame matrices/bounds evidence is in `validation/matrices-and-posed-bounds.json`.
