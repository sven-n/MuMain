...
def review(name, original_count):
    ...
    draw.text((30, 25), f'{name} / FIRE PROPS AND DUNGEON ENTRANCE', font=title, fill='#e1e2d3')
    ...
    text = [f'{original_count} → {report["triangles"]} triangles', '512 RGB / 256 RGBA paintings',
            'Original bind bounds retained', 'Bone order / parents / action: EQUIVALENT',
            'Geometry: ' + ('DIFFERENT' if name == 'FireLight01' else 'EQUIVALENT'), 'Scale bar: 190 game units tall']
    for index, line in enumerate(text):
        draw.text((1210, 680 + 42*index), line, font=label, fill='#dedecf')
    draw.text((30, 1040), 'Sources include REF_ORIGINAL. Procedural runtime fire is absent; this is an offline diffuse/geometry study.', font=label, fill='#a8b5af')
    sheet.save(folder / 'review/comparison.jpg', quality=94)


for name, count in [('FireLight01', 48), ('FireLight02', 34), ('Bonfire01', 110), ('DoungeonGate01', 333)]:
    review(name, count)
