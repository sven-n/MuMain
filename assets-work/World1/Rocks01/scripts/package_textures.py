...
from config import DIMENSION, REPOSITORY, ROOT, TEXTURES
...
def editable_source(name, diffuse, mask, final):
    document = Element('image', {'w': str(DIMENSION), 'h': str(DIMENSION), 'name': name})
    stack = SubElement(document, 'stack')
    layers = [('Painted color with preserved alpha', final, 'visible'),
              ('Original alpha field - bilinear 16x' if name == 'ston02' else 'Opaque alpha - no cutouts', mask.convert('RGBA'), 'hidden'),
...
def package(name):
    ...
    color = raw.resize((DIMENSION, DIMENSION), Image.Resampling.LANCZOS)
    ...
    return {'size': list(final.size), 'mode': final.mode, 'original_size': list(original.size),
            ...
            'fringe_color_source': 'Continuous botanical painting across entire canvas, including hidden pixels',
            'alpha_mean_original': sum(original.getchannel('A').getdata()) / (original.width * original.height),
            'alpha_mean_final': sum(alpha.getdata()) / (DIMENSION * DIMENSION)}


def package_rock():
    color = Image.open(ROOT / 'textures/ston01-generated.png').convert('RGB')
    color = color.resize((DIMENSION, DIMENSION), Image.Resampling.LANCZOS)
    color.save(ROOT / 'textures/ston01.png')
    color.save(ROOT / 'textures/ston01.jpg', quality=97, subsampling=0)
    editable_source('ston01', color, Image.new('L',color.size,255), color.convert('RGBA'))
    subprocess.run([sys.executable,str(REPOSITORY/'tools/mu_texture.py'),'wrap',
                    str(ROOT/'textures/ston01.jpg'),'--out',str(ROOT/'textures/ston01.OZJ')],check=True)
    subprocess.run([sys.executable,str(REPOSITORY/'tools/mu_texture.py'),'check',
                    str(ROOT/'textures/ston01.OZJ')],check=True)
    return {'size':list(color.size),'mode':'RGB','alpha':'opaque','original_size':[128,128]}
