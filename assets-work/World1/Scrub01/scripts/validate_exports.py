...
TOLERANCE = 0.0001
BOUND_ROUNDTRIP_TOLERANCE = 0.001  # Blender/SMD float serialization, well below bmdconv's 0.05.
...
def geometry(original, replacement, name):
    ...
    assert uv_collapsed[0] == uv_collapsed[1]
...
def validate(name):
    ...
    comparison = run([CONVERTER,'compare',root/'original'/f'{name}.bmd',root/'exports'/f'{name}.bmd'])
    assert 'EQUIVALENT' in comparison['stdout']
    commands.append(comparison)
    (root/'validation/compare.txt').write_text(comparison['stdout'])
    for texture in sorted((root/'exports').glob('*.OZT')):
    ...
    report = {'status':'PASS OFFLINE', 'client_verified':False, 'geometry':geometry(original,replacement,name),
              'rig':local_rig(original,replacement,name), 'full_compare':'EQUIVALENT at converter default 0.05 unit tolerance',
