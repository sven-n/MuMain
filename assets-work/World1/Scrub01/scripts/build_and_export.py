"""Retain authored vegetation forms and rigs; replace diffuse artwork through official export."""
import hashlib
import json
...
def new_material(original):
    ...
    texture.image = bpy.data.images.load(str(ROOT / 'textures' / name))
    texture.image.pack()
...
def build(name):
    ...
    report = {'geometry_uv_weights_transforms_unchanged': True,
    ...
    for material_name in report['material_order']:
        texture_name = Path(material_name).stem + '.OZT'
        shutil.copy2(ROOT / 'textures' / texture_name, root / 'exports' / texture_name)
