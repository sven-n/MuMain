...
import re
from config import ASSETS, BLENDER, CONVERTER, FROZEN, KEYS, MATERIALS, REPOSITORY, ROOT
...
def export(name):
    folder = ROOT / name
    command = [BLENDER, '-b', '--python-exit-code', '1', str(folder / 'source.blend'), '--python',
    ...
    for material in MATERIALS[name]:
        texture = Path(material).stem + ('.OZT' if material.endswith('.tga') else '.OZJ')
...
def validate(name):
    ...
    check = subprocess.run([sys.executable,str(REPOSITORY/'tools/mu_texture.py'),'check',
                            *map(str,sorted((folder/'exports').glob('*.OZ*')))],cwd=REPOSITORY,capture_output=True,text=True,check=True)
    texture_check = check.stdout
    (reports / 'texture-check.txt').write_text(texture_check)
    summary = dict(status='PASS: offline', client_verified=False, geometry='Original trunk/crown geometry retained; full BMD comparison EQUIVALENT',
...
for filename in FROZEN:
    assert (ROOT/'textures/final'/filename).read_bytes() == (ROOT/'textures/original'/filename).read_bytes()
    assert (REPOSITORY/'src/bin/Data/Object1'/filename).read_bytes() == (ROOT/'textures/original'/filename).read_bytes()
assert (REPOSITORY/'src/bin/Data/Object1/Tree11.bmd').read_bytes() == (ROOT/'Tree11/original/Tree11.bmd').read_bytes()
