...
RECESS_DEPTH = .4
PANEL_BEVEL_WIDTH = 1.0
MAX_INSET_FRACTION = .06
PROP_TRIANGLE_BUDGET = 1500

PAIRS = {'StoneStatue01': [(2,3)], 'StoneStatue02': [], 'StoneStatue03': [], 'SteelStatue01': [(10,11),(12,13),(14,15),(16,17)], 'Tomb01': [(2,3),(4,5),(6,7),(8,9)], 'Tomb02': [(2,3),(4,5),(6,7),(8,9)], 'Waterspout01': []}
...
def perimeter(original, pair):
    ...
    edges, points = [], {}
    for face in faces:
        assert original.data.materials[face.material_index].name.endswith('.jpg')
...
def repair_head_cap(obj):
    """Replace the old narrow UV sliver only on the six top-cap triangles."""
    faces=(32,33,69,70,93,94)
    points=[obj.data.vertices[i].co for index in faces for i in obj.data.polygons[index].vertices]
    low=[min(v[i] for v in points) for i in (0,1)]
    high=[max(v[i] for v in points) for i in (0,1)]
    for index in faces:
        face=obj.data.polygons[index]
        for loop in face.loop_indices:
            vertex=obj.data.vertices[obj.data.loops[loop].vertex_index].co
            obj.data.uv_layers[0].data[loop].uv=(.63+.34*(vertex.x-low[0])/(high[0]-low[0]),
                                               .56+.34*(vertex.y-low[1])/(high[1]-low[1]))
    return list(faces)
...
def validate_source(root, obj, rig, original, bounds, evidence):
    ...
    report = dict(status='PASS: source checks; exported engine checks separately required',
                  ...
                  uv='One finite UVMap; original UVs and repeats retained except six reprojected head cap triangles; new panel coordinates interpolated',
                  skinning='Exactly one original non-dummy bone with weight 1 per vertex',
                  alpha='Opaque RGB stone atlases; frozen fountain water remains opaque texture for original BlendMesh3',
                  uv_reprojected_faces=[32,33,69,70,93,94] if root.name=='StoneStatue02' else [],
                  uv_reprojection='Head top cap only: planar projection into the same atlas plain stone region; fixes legacy 0.023-high UV sliver' if root.name=='StoneStatue02' else None,
