...
def audit(name):
    ...
    original_points={tuple(vertex.co) for obj in ref.objects for vertex in obj.data.vertices}
    final_points={tuple(vertex.co) for obj in active for vertex in obj.data.vertices}
    assert original_points <= final_points,name+' original connection vertex lost'
    ...
    result={'status':'PASS','reference_geometry_uv_weights_transform':'identical to preserved import',
            ...
            'original_connection_vertices_exact':len(original_points),
