"""Package six shared paintings, retaining atlas identities and frozen alpha containers."""
import json
...
SIZES = {'bridge_01': (1024, 512), 'tree_04': (512, 512), 'tile_ston06': (512, 512), 'tile_wood02': (512, 512), 'tile_ston04': (512, 512), 'tile_02': (512, 512)}
JPEG_QUALITY = 95
...
def main():
    for original in sorted(ROOT.glob('*/original')):
        for texture in list(original.glob('*.OZJ')) + list(original.glob('*.OZT')):
            ...
            if name in SIZES and texture.suffix == '.OZJ':
    ...
    records = [dict(name=name, size=list(size), tool='Built-in imagegen',
