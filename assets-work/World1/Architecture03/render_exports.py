...
EFFECT_NAMES = ('House03', 'House05', 'House04', 'HouseWall02')
ACTION_FRAMES = {'House05': (0, 14, 29), 'House04': (0, 19, 39), 'Tent01': (0, 17, 35)}
SCROLL_MATERIALS = {'House05': 'ston02.jpg', 'House04': 'tile_space01.jpg'}
...
def approximate_effects(name):
    ...
    filename = SCROLL_MATERIALS.get(name, 'light_02.jpg')
    for material in bpy.data.materials:
        ...
        emission.inputs['Strength'].default_value = 1 if name in SCROLL_MATERIALS else .55
...
def action_views(root, stage):
    for index, frame in enumerate(ACTION_FRAMES.get(root.name, ())):
        ...
        if root.name in SCROLL_MATERIALS:
            bpy.data.materials[SCROLL_MATERIALS[root.name]].node_tree.nodes['OFFLINE_V_SCROLL'].inputs[1].default_value[1] = -index / 3
        render(root / f'review/action-{stage}-{frame}.png')
    bpy.context.scene.frame_set(0)
    if root.name in SCROLL_MATERIALS:
        bpy.data.materials[SCROLL_MATERIALS[root.name]].node_tree.nodes['OFFLINE_V_SCROLL'].inputs[1].default_value[1] = 0
...
def frame_asset(root, stage, bounds):
    review.open_model(root, stage, bounds)
    if root.name == 'HouseWall04':
...
def review_asset(name):
    ...
    (root / 'review/render-context.json').write_text(json.dumps(dict(
        ...
        additive='OFFLINE APPROXIMATION: transparent plus image emission; light_02 midpoint brightness 0.55, ston02 and tile_space01 brightness 1. Neutral diffuse images also retained.',
        animation_frames=ACTION_FRAMES.get(name, ()),
        porthole_scroll='House04 and House05 pose columns pair recorded action frames with illustrative V offsets 0/-1/3/-2/3. Actual engine scroll is world-time driven; mesh UVs are unmodified.',
