"""Preserve and import six new architecture assets and three accepted readonly consumers; never replace baselines."""
import json
import os
...
def preserve(name, record):
    ...
    info = subprocess.check_output([CONVERTER, 'info', str(original / f'{name}.bmd')], text=True)
    (original / 'info.txt').write_text(info)
