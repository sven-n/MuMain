...
def main():
    ...
    for name in OWNED:
        consumers = [model for model, record in inventory['models'].items() if any(Path(texture).stem == name for texture in record['textures'])]
    ...
    for name in NAMES:
        ...
        for path in sorted((folder / 'textures').iterdir()):
            ...
            with Image.open(path) as image:
                ...
                if path.suffix == '.tga':
                    with Image.open(folder / 'original' / path.name) as old:
                        original_alpha = old.getchannel('A').resize(image.size, Image.Resampling.NEAREST)
                    assert image.getchannel('A').tobytes() == original_alpha.tobytes()
                    assert path.read_bytes()[16:18] == bytes((32, 8))
                    images[path.name]['original_alpha_nearest4x_exact'] = True
    ...
    assert (ROOT / 'HouseEtc02/exports/HouseEtc02.bmd').read_bytes() == (REPOSITORY / 'src/bin/Data/Object1/HouseEtc02.bmd').read_bytes() == (ROOT / 'HouseEtc02/original/HouseEtc02.bmd').read_bytes()
    print('PASS: immutable originals; six textures cover five owned models plus readonly annex; UV/skin/alpha/effect contracts')
