"""Validate architecture exports and the unchanged accepted annex against complete original rigs/actions."""
import hashlib
...
EFFECTS = {'House03': (4, 'light_02.jpg'), 'House04': (8, 'tile_space01.jpg'), 'HouseWall02': (4, 'light_02.jpg')}
...
def check_model(root):
    ...
    expected = 'EQUIVALENT' if root.name == 'HouseEtc02' else 'DIFFERENT'
    assert expected in comparison.stdout
    assert (comparison.returncode == 0) == (expected == 'EQUIVALENT')
    if root.name == 'HouseEtc02':
        assert old.read_bytes() == new.read_bytes(), 'Accepted annex BMD must be byte-identical'
...
def validate(name):
    ...
    summary.update(status='PASS: offline engine validation', client_verified=False, action_metadata=metadata,
                   skeleton_actions='EQUIVALENT', readonly_compatibility=name == 'HouseEtc02',
