...
def review(name, original_count):
    ...
    draw.text((30, 25), f'{name} / WELL AND EARTHENWARE', font=title, fill='#e1e2d3')
    ...
    text = [f'{original_count} → {report["triangles"]} triangles', '512 RGB material paintings',
            'Original bind bounds retained', 'Bone order / parents / action: EQUIVALENT',
            'Geometry: ' + ('EQUIVALENT' if name in ('Well02','Carriage03') else 'DIFFERENT'), 'Scale bar: 190 game units tall']
    for index, line in enumerate(text):
        draw.text((1210, 680 + 42*index), line, font=label, fill='#dedecf')
    draw.text((30, 1040), 'Sources include REF_ORIGINAL. Original action and ground contacts retained. Offline diffuse/geometry study.', font=label, fill='#a8b5af')
    sheet.save(folder / 'review/comparison.jpg', quality=94)


for name, count in [('Well01',405),('Well02',123),('Well03',194),('Well04',388),('Carriage03',538)]:
    review(name, count)
