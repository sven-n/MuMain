"""Install seven validated owned files only into this isolated source worktree."""
import hashlib
import json
...
TEXTURES = ('well.OZJ','jar_01.OZJ','tub.OZJ')
...
def main():
    assert subprocess.check_output(['git', 'branch', '--show-current'], cwd=ROOT, text=True).strip() == 'codex/lorencia-wells'
    ...
    compatibility = HERE / 'Carriage03'
    revision = (compatibility / 'original-revision.txt').read_text().strip()
    expected_cart = subprocess.check_output(['git','show',revision+':src/bin/Data/Object1/Carriage03.bmd'],cwd=ROOT)
    assert (compatibility/'exports/Carriage03.bmd').read_bytes() == expected_cart
    assert json.loads((compatibility/'validation/source-export-audit.json').read_text())['result'] == 'PASS'
    protected = {}
    ...
    record = dict(result='PASS', baseline='cd7ed604', runtime_modified=False, client_verified=False,
