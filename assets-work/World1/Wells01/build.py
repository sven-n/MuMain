"""Refine pottery curvature within original footprints and repaint well materials."""
import json
import math
from pathlib import Path
import sys

import bpy
import bmesh
from mathutils import Vector

sys.dont_write_bytecode = True
HERE = Path(__file__).resolve().parent
sys.path.insert(0, str(HERE))
from prepare import ASSETS
sys.path.insert(0, str(HERE.parent / 'StaticBatch01'))
from geometry import collection
from review_scene import mesh_bounds


def preserve_original():
    reference = collection('REF_ORIGINAL')
    rig = next(obj for obj in bpy.context.scene.objects if obj.type == 'ARMATURE')
    for obj in list(bpy.context.scene.objects):
        if obj.type != 'MESH' or obj.get('mu_helper'):
            continue
        for owner in list(obj.users_collection):
            owner.objects.unlink(obj)
        reference.objects.link(obj)
        obj['mu_reference'] = True
    reference.hide_render = True
    reference.hide_viewport = True
    for material in list(bpy.data.materials):
        material.name = 'REF_' + material.name
    return rig, reference


def repaint(obj):
    for index, original in enumerate(list(obj.data.materials)):
        material = original.copy()
        texture_name = original.get('mu_texture') or original.name.removeprefix('REF_')
        material.name = texture_name
        material['mu_texture'] = texture_name
        for node in material.node_tree.nodes:
            if node.type == 'TEX_IMAGE' and node.image:
                node.image = bpy.data.images.load(str(HERE / 'textures/final' / texture_name), check_existing=False)
        obj.data.materials[index] = material


ROUNDING = .55

def round_pottery(obj):
    mesh = bmesh.new()
    mesh.from_mesh(obj.data)
    deform = mesh.verts.layers.deform.active
    targets = [f for f in mesh.faces if obj.data.materials[f.material_index].get('mu_texture') == 'jar_01.jpg']
    if not targets:
        mesh.free()
        return dict(pottery_triangles=0)
    original = {v: v.co.copy() for v in mesh.verts}
    groups = {}
    for vertex in mesh.verts:
        group = next(iter(vertex[deform].keys()))
        groups.setdefault(group, []).append(vertex.co.copy())
    bounds = {g: ([min(v[k] for v in vs) for k in range(3)], [max(v[k] for v in vs) for k in range(3)]) for g, vs in groups.items()}
    uv = mesh.loops.layers.uv.active
    collapsed = set()
    for face in mesh.faces:
        a, b, c = [loop[uv].uv for loop in face.loops]
        if abs((b.x-a.x)*(c.y-a.y)-(b.y-a.y)*(c.x-a.x)) < 2e-10:
            collapsed.update(face.edges)
    # Original solid-color cap triangles have zero UV area; keep them intact.
    edges = list({edge for face in targets for edge in face.edges if edge not in collapsed})
    bmesh.ops.subdivide_edges(mesh, edges=edges, cuts=1, use_grid_fill=True, smooth=ROUNDING)
    for vertex in mesh.verts:
        if vertex in original:
            vertex.co = original[vertex]
            continue
        weights = vertex[deform]
        assert len(weights) == 1
        group = next(iter(weights.keys()))
        weights[group] = 1
        low, high = bounds[group]
        for axis in range(3):
            vertex.co[axis] = max(low[axis], min(high[axis], vertex.co[axis]))
    bmesh.ops.triangulate(mesh, faces=list(mesh.faces))
    for face in mesh.faces:
        face.smooth = obj.data.materials[face.material_index].get('mu_texture') == 'jar_01.jpg'
    triangles = len(mesh.faces)
    mesh.to_mesh(obj.data)
    mesh.free()
    obj.data.update()
    return dict(original_anchors=len(original), final_triangles=triangles, rounding=ROUNDING)


def build(name):
    folder = HERE / name
    bpy.ops.wm.open_mainfile(filepath=str(folder / 'original/source.blend'))
    bpy.context.scene.frame_set(0)
    rig, reference = preserve_original()
    target = collection('EXPORT_' + name)
    objects, edits = [], []
    for original in reference.objects:
        obj = original.copy()
        obj.data = original.data.copy()
        obj.name = name + '_Production'
        obj['mu_reference'] = False
        target.objects.link(obj)
        obj.hide_render = False
        obj.hide_viewport = False
        edits.append(round_pottery(obj))
        repaint(obj)
        objects.append(obj)
    before, after = mesh_bounds(list(reference.objects)), mesh_bounds(objects)
    assert max(abs(a-b) for aa,bb in zip(before,after) for a,b in zip(aa,bb)) < .0001
    report = dict(bounds_before=before, bounds_after=after,
                  triangles=sum(len(o.data.polygons) for o in objects), edits=edits,
                  bone_order=list(rig['mu_bone_order']), action_meta=rig['mu_action_meta'].to_dict(),
                  retained_geometry=name in ('Well02','Carriage03'),
                  note='Original pottery anchors retained; new curved edge points constrained to each original bone AABB')
    (folder / 'validation/blender.json').write_text(json.dumps(report, indent=2) + '\n')
    bpy.ops.file.pack_all()
    bpy.context.preferences.filepaths.save_version = 0
    bpy.ops.wm.save_as_mainfile(filepath=str(folder / 'source.blend'))


for asset in (sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else ASSETS):
    build(asset)
