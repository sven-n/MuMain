"""Dress selected wood panel interiors while retaining every original boundary corner."""
import json
import math
...
PAIRS = {'Sign01': [(12, 13), (14, 15)], 'Sign02': [(0, 1), (2, 3)], 'Curtain01': [], 'StoneWall04': [], 'StoneWall06': []}
...
def perimeter(original, pair):
    ...
    edges, points = [], {}
    for face in faces:
        assert original.data.materials[face.material_index].name.endswith('notice.jpg'), original.data.materials[face.material_index].name
...
def create_export(root, original, rig, geometry):
    ...
    data = bpy.data.meshes.new(name + '_Dressed_Wood')
    data.from_pydata(vertices, [], faces)
    data.update()
    obj = bpy.data.objects.new(name + '_Dressed_Wood', data)
...
def retain_high_poly(obj):
    ...
    bevel = clone.modifiers.new('Optional_Dressed_Wood_Bake', 'BEVEL')
    bevel.width = .12
    bevel.segments = 3
...
def validate_source(root, obj, rig, original, bounds, evidence):
    ...
    report = dict(status='PASS: source checks; exported engine checks separately required',
                  ...
                  alpha='Opaque notice and pole atlases; original scalar masks retained for four alpha textures',
