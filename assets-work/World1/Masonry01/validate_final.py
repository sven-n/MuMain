...
def main():
    ...
    for name in OWNED:
        consumers = [model for model, record in inventory['models'].items() if f'{name}.jpg' in record['textures']]
        assert set(consumers).issubset(NAMES)
        ownership[name] = consumers
    for name in NAMES:
        folder = ROOT / name
        for path in (folder / 'original').iterdir():
            if path.suffix not in ('.bmd', '.OZJ'):
                continue
        ...
        images = {}
        for path in (folder / 'textures').glob('*.jpg'):
            with Image.open(path) as image:
                assert image.mode == 'RGB' and all(x <= 1024 and x & (x - 1) == 0 for x in image.size)
                images[path.name] = dict(dimensions=list(image.size), mode=image.mode, alpha=False)
    ...
    print('PASS: immutable original bytes, four textures wholly owned by six models, exported UV/skin/material/alpha contracts')
