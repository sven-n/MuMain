...
import re
from config import PRODUCTION_ASSETS, ASSETS, BLENDER, CONVERTER, CONTAINERS, FROZEN, KEYS, MATERIALS, REPOSITORY, ROOT
...
def converter_run(*arguments, check=True):
    result = subprocess.run([CONVERTER,*map(str,arguments)],cwd=REPOSITORY,capture_output=True,text=True)
    if check and result.returncode: raise RuntimeError(result.stdout+result.stderr)
    return result
shared.run=converter_run
...
def export(name):
    ...
    if name not in PRODUCTION_ASSETS:
        destination=folder/'validation/official-roundtrip.bmd'
        shutil.copy2(folder/f'exports/{name}.bmd',destination)
        shutil.copy2(folder/f'original/{name}.bmd',folder/f'exports/{name}.bmd')
...
def validate(name):
    ...
    assert ('DIFFERENT' if name in PRODUCTION_ASSETS else 'EQUIVALENT') in comparison.stdout
    ...
    summary = dict(status='PASS: offline', client_verified=False, geometry='Intentional geometry rebuild, full compare DIFFERENT' if name in PRODUCTION_ASSETS else 'Readonly accepted BMD retained byte-identical, full compare EQUIVALENT',
