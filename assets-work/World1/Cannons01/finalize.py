...
from config import PRODUCTION_ASSETS, ASSETS, BASELINE, CONTAINERS, IDENTITIES, MATERIALS, OWNED_TEXTURES, REPOSITORY, ROOT
BACKGROUND = (29,33,34)
def panel(path,size):
...
def original_hashes():
    record=json.loads((ROOT/'original-provenance.json').read_text())
    for name,item in record.items():
        original=subprocess.check_output(['git','show',item['source_revision']+':src/bin/Data/Object1/'+name],cwd=REPOSITORY)
        assert (ROOT/item['archive']).read_bytes()==original
...
def install():
    files = {f'src/bin/Data/Object1/{name}.bmd':ROOT/name/f'exports/{name}.bmd' for name in PRODUCTION_ASSETS}
...
def all_consumer_sheet():
    sheet=Image.new('RGB',(1680,1440),BACKGROUND)
    draw=ImageDraw.Draw(sheet)
    for index,name in enumerate(ASSETS):
        x=(index%3)*560;y=(index//3)*360
        draw.text((x+10,y+10),name+' | BEFORE / AFTER | OFFLINE',fill='white')
        for column,stage in enumerate(('before','after')):
            sheet.paste(panel(ROOT/name/'review'/(stage+'.png'),(280,310)),(x+280*column,y+40))
    sheet.save(ROOT/'review/all-consumers.jpg',quality=94)
...
def main():
    ...
    for name in ASSETS:
        review_sheet(name)
        for filename in ('summary.json','source-audit.json','geometry-UV-contacts.json','local-motion.json','raw-normal-bindings.json','all-key-posed-bounds.json'):
