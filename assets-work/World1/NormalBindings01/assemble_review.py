...
def review(name, original_count):
    ...
    draw.text((30, 25), f'{name} / RETAINED GEOMETRY / PAINTED MATERIALS', font=title, fill='#e1e2d3')
    ...
    draw.text((30, 1040), 'Sources include REF_ORIGINAL. Original raw normal bindings and all action keys retained. Offline diffuse/geometry study.', font=label, fill='#a8b5af')
    sheet.save(folder / 'review/comparison.jpg', quality=94)


for name, count in [('Grass02',120),('Tree12',393),('Tree13',393)]:
    review(name, count)
