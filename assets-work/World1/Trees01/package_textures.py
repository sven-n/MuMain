...
SIZES = {'Tree_a': (512,512), 'tree_06': (512,512), 'tree':(512,512), 'tree_02':(512,256)}
ALPHA_TEST = 64
...
def bleed_transparent_rgb(image):
    """Nearest visible RGB fills transparent texels; generated alpha stays identical."""
...
def package_alpha(name, size):
    ...
    report = dict(original=alpha_statistics(original), generated=alpha_statistics(raw),
                  final=alpha_statistics(image), fringe='Nearest visible RGB padded under alpha=0; alpha unchanged')
...
for texture, dimensions in SIZES.items():
    if texture in ('Tree_a','tree_06'):
        package_alpha(texture, dimensions)
    else:
        package_opaque(texture, dimensions)
for suffix in ('OZJ','jpg'):
    shutil.copy2(ROOT / f'textures/original/tree_03.{suffix}', ROOT / f'textures/final/tree_03.{suffix}')
