...
def audit(name):
    ...
    tested=0
    if name=='Object22':
        z=min(p[2] for p in old);rim=[p for p in old if abs(p[2]-z)<TOLERANCE];cx=sum(p[0] for p in rim)/len(rim);cy=sum(p[1] for p in rim)/len(rim);rim.sort(key=lambda p:math.atan2(p[1]-cy,p[0]-cx))
        for a,b in zip(rim,rim[1:]+rim[:1]):
            for i in range(21):
                p=[x+(y-x)*i/20 for x,y in zip(a,b)];assert any(point_on_triangle(p,t) for t in new_tri);tested+=1
    report=dict(status='PASS',tolerance=TOLERANCE,original_convex_support_planes=len(planes),maximum_outward_support_excess=excess,actual_placements=records,underside_perimeter_surface_samples=tested,continuous_underside='Original4triangles independently exactly matched in authored-export-contract.json' if name=='Object22' else None,method='All original convex support planes contain final geometry; all actual tilted placement bounds unchanged; underside perimeter sampled over full edges in addition to exact original face matching')
    (folder/'validation/contact-envelope.json').write_text(json.dumps(report,indent=2)+'\n');print(name,'contact/envelope PASS',excess)

for name in os.environ.get('COFFIN_NAMES','Object22').split(','):audit(name)
