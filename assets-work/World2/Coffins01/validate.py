...
def audit(name):
    ...
    assert struct.unpack_from('<3h',original,32)==struct.unpack_from('<3h',final,32)==(1,1,1)
    logs=[]
    logs.append(run('validate',new/f'{name}.smd'))
    for root in (old,new):
        text=(root/f'{name}.smd').read_text();assert 'wood01.jpg' in text
    ...
    hashes=json.loads((ROOT/'frozen-texture.json').read_text())
    ...
    report=dict(status='PASS',raw32byte_name_equal=True,name_hex=original[:32].hex(),mesh_bone_action_counts=[1,1,1],keys=1,maximum_motion_component_delta=difference,texture_frozen=True,sha256=hashlib.sha256((folder/'exports'/f'{name}.bmd').read_bytes()).hexdigest())
