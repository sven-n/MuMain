...
PROTECTED = [index for index in range(90) if index not in RELIEF_RANGE]
    ...
    for face,(_,corners,old_index) in zip(mesh.polygons,faces):
        for corner,loop in enumerate(face.loop_indices):
            if old_index is not None:
                ...
                normals[loop]=sum((n for n in nearby if n.dot(face.normal)>.90),Vector()).normalized()
...
def main():
    ...
    (folder/'validation/source.json').write_text(json.dumps(dict(triangles=len(mesh.polygons),
        ...
        design='Broad connected planar brow ridge and recessed eye sockets; exact original nose, lower mask, structural block and seams'),indent=2))
