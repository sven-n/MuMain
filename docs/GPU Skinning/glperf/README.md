...
- **Purpose**: Replaces the client's fixed request for exactly GL 3.3 core with a descending `{4,5} → {4,3} → {3,3}` attempt chain, and publishes an `RHI::Caps` struct (`bufferStorage`, `vertexAttribBinding`, `programBinary`, `timerQuery`, UBO offset alignment, max uniform block size) so later tasks can branch on real hardware capability instead of assuming a version. Adds a `config.ini [graphics.opengl] MaxGLVersion` override as a rollback path.
