...
| Milestone | Subsystem | Focus / Objective | Status |
|---|---|---|---|
| **[GLP-16](#glp-16--bucket-terrain-tiles-by-texture-pair)** | Terrain | Bucket visible terrain tiles by `(baseTexture, overlayTexture, waterFlags)` into one draw per bucket. | Implemented — build-verified only; visual identity check not yet performed |
...
- **Scope**: `Render/Terrain/ZzzLodTerrain.cpp`, possibly `Render/Shaders/TerrainShader.h`/`.cpp`.
...
- **Status**: Implemented, build-verified only. Pre-implementation verification confirmed the terrain pass is opaque and depth-tested throughout (making the draw-order change from bucketing safe) and traced every side effect and consumer carefully before touching them. A dev-machine capture showed terrain draw calls dropping 1,226 → 48 (25.5×) at the same test spot with no reported corruption in a general pass, but the task's own verification section — which calls a before/after screenshot comparison "practical and conclusive" — has not been executed: multi-layer blending at bucket-crossing tile boundaries, water scroll/shoreline behavior, the Atlans/Doppelganger3 overlay substitution, Crywolf splash-particle rate, grass maps, and dev editor mode are all unverified, and Intel HD 530 frame-time (the actual target of this task) has not been measured.
