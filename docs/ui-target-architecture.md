...
| Buttons                                        | 4 implementations, split further into native/RmlUi contexts                                                                                                                                                                                          | **`base.rcss`'s `.btn` + a documented variant convention, going forward.** The evolved `mu::ui::window::CButton` (after replacing its `RenderImage()` rendering path and hand-rolled `CheckMouseIn(m_Pos.x, ...)` hit-tests with `CSprite`/`WindowGeometry`, and removing its macro-forked behavior) is a **transitional bridge for the still-native population only**, not a coequal permanent family | No hybrid window found actually needs a *primary* native button — every documented case (`CMsgWin`, `CSysMenuWin`) is a redundant click-detector already being retired; the native family should shrink toward zero, not stand alongside RmlUi as a second permanent target |
...
6. Retire `mu::ui::window::CButton`'s `RenderImage()` path in favor of `CSprite`, and its
   ...
   `WindowGeometry` (that function itself is `WindowCommon.h`'s own, not a `UIControls.h`
   dependency — see Section A's correction — so this is about owning a rect object instead of
   hand-rolling the check, not about escaping the legacy toolkit) — steps toward shrinking the
   transitional native family, not toward hardening it as permanent.
