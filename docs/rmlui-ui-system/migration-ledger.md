...
| Component | Category | Status | Target shape / primitive | Detail pointer |
|---|---|---|---|---|
| `COptionWindow` | `CObject`-tier | Built (RelWithDebInfo), in-game verification pending | RmlUi-only 2D, via `window_shell` (extended with a `positioned`/drag mode, its first non-centered consumer) | 5 checkboxes (`.checkbox-row`), 3 sliders (RmlUi native `<input type="range">`, same `WidgetSlider` sub-elements `generic_confirm_dialog.rcss`'s scrollbar already proves), 3 dropdowns (RmlUi native `<select>`/`<option>` — first use in this codebase, unverified at runtime), drag-by-header-rail persisted via `GameConfig::Get/SetWindowPosition(L"option_window", ...)`. **First in-game pass (2026-09-17) found two real bugs, both fixed**: (1) `window_shell`'s `positioned` mode originally also multiplied a `root_scale` transform (`CCharacterInfoWindow`'s older reference-resolution convention) onto `window_shell`'s own already-real-dp chrome, badly oversizing the whole dialog and throwing off internal layout — removed `root_scale` entirely from `window_shell`/`GenericMenuDialog`/`OptionWindow`, `#panel` resized to a genuine real-dp `260x380` (was native's reference-pixel `190x419`), position now computed directly in real device pixels (`RestoreDefaultOrUserPosition()`), no scale transform. (2) The resolution `<select>` fired a spurious `change` event while settling onto its initial `data-value`-synced index, silently re-applying index 0 (the smallest listed resolution) and force-downgrading the real window to it on open. An initial "no-op unless the index actually changed" guard (`ApplyLanguage()`/`ApplyFont()`'s own pattern) turned out **not** to be sufficient — a second in-game pass confirmed the window still got forced to 640×480 on boot even with that guard in place. Root cause: RmlUi's `WidgetDropDown` (the `<select>` backing class) re-fires `change` a few times of its own accord while it settles its `data-for` option list against the freshly-populated `data-value` index — and the spurious index it lands on can legitimately differ from the already-correct `m_iResolutionIndex`, defeating a same-value guard. Fixed by ignoring all three selects' change callbacks (resolution/language/font) for a short settle window (`m_rmlSyncCount < kRmlSelectSettleFrames`, 5 `SyncRmlModel()` calls) after `BuildRmlUi()` — real user input can't land this soon after `Create()` since the window isn't shown yet. Rebuilt clean after all three fixes; still needs a live-server look to confirm the visual fix and the native `<select>`/`<input type="range">` rendering/behavior — a `data-for` custom dropdown remains the documented fallback if `<select>` doesn't hold up. |
