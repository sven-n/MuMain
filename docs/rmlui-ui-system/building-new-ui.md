...
|---|---|---|---|
| Sprite widgets | `CButton : CSprite`, `CGaugeBar`, `CWin`, `CWinEx : CWin` | `UI/Widgets/{Button,GaugeBar,Win,WinEx}.h` | **Being retired.** `CWin`/`CWinEx` have zero live *subclasses* — every window that still touches them is `mu::ui::window::CObject`-derived and only held one as a composed member. `CSysMenuWin`/`CCharMakeWin` have had theirs removed (both were provably dead: never rendered, and any real state the redundant sprite objects held has moved to a plain struct read by RmlUi's data binding). `CServerSelWin::m_winDescription` (`CWinEx`) is the one still-alive member — its height comes from `CWinEx::SetLine()`'s asset-derived pixel math (via `GetTexHeight()`, the *full* loaded texture's height, not the small sub-rect declared in code), which isn't safely reproducible without inspecting the actual `.OZJ` asset at runtime, so it's deliberately left alone rather than guessed at. `CGaugeBar` and the sprite-tier `CButton` remain genuinely live too, via that same window's server/gauge list — retiring those means rewriting that screen's rendering, not a mechanical port. Don't add a new consumer of any class in this row. |
