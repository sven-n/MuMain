...
// nullptr for values outside the table so callers can fall back to the raw
// number. The static_assert keeps the table in sync if the enum grows.
...
{
    const wchar_t* modelName = GetMonsterModelName(Type);
    g_ErrorReport.Write(L"OpenMonsterModel(%ls = %d)\r\n", modelName ? modelName : L"UNKNOWN", Type);

    if (!modelName)   // out-of-range Type: already logged above, don't index Models[]
        return;
