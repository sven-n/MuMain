...
void Draw(RHI::Topology topology, uint32_t vertexCount, uint32_t firstVertex)
{
    ...
    switch (topology)
    {
    case RHI::Topology::TriangleList: topo = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST; break;
    case RHI::Topology::LineList:     topo = D3D11_PRIMITIVE_TOPOLOGY_LINELIST;     break;
    case RHI::Topology::LineStrip:    topo = D3D11_PRIMITIVE_TOPOLOGY_LINESTRIP;    break;
    default: return;
    }
...
void BindTexture(RHI::TextureHandle handle, int slot)
{
    ...
    {
        if (ID3D11SamplerState* sampler = GetOrCreateSampler(it->second.filter, it->second.wrap))
...
// inside the RHI_D3D11_AVAILABLE block above) -- PlanarShadowShader's D3D11 path needs it. ----
