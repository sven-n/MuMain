...
Rml::TextureHandle RmlUiRenderInterface::LoadTexture(Rml::Vector2i& texture_dimensions, const Rml::String& source)
{
    ...
    // sniffing). This is required, not just a cache-sharing convenience: this game's actual art
    // assets are almost entirely OZT/OZJ, a custom container format SDL_image has no reader for,
    // so the base class's own LoadTexture cannot load any of this game's existing images at all.
    //
    ...
    // (symptom: return to the login screen and the panel renders garbled, then crashes).
    // LoadImageExclusive always creates its own independent entry, immune to any other scene's
    // slot churn, at the cost of a small amount of duplicate VRAM for these two small UI textures.
...
void RmlUiRenderInterface::ReleaseTexture(Rml::TextureHandle texture_handle)
{
    ...
    // glyph atlases). Let it manage that lifetime itself.
