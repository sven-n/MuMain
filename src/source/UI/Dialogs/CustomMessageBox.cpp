...
CALLBACK_RESULT mu::ui::window::CStorageLockCheckKeyPadMsgBoxLayout::OkBtnDown(class CMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
    ...
    if (pMsgBox->GetInputSize() == pMsgBox->GetInputLimit())
    {
        ...
        if (pMsgBox->IsCheckInput() == true)
        {
            ...
            // non-numeric-restricted Mode::Text WEBZEN.COM password entry, ported 2026-09-14. The
            // 4-digit PIN just entered above (wInputNumber) has no equivalent field on
            // GenericDialogConfig -- captured directly in the closure instead of the native
            // SetPassword()/GetPassword() round-trip through the MsgBox instance.
            mu::ui::window::GenericDialogConfig cfg;
            cfg.buttons = mu::ui::window::GenericDialogConfig::ButtonSet::OkCancel;
            cfg.lines = {
            ...
            cfg.onPrimary = [wInputNumber]
            {
                ...
                SocketClient->ToGameServer()->SendSetVaultPin(wInputNumber, MU_C16(strText.c_str()));
            };
            mu::ui::window::g_pGenericConfirmDialog->Show(std::move(cfg));
