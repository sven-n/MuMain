...
namespace UI::Skills::Tooltip
{
    ...
    // The same content resolution Render() itself performs (pet-command dispatch via
    // GIPetManager, then BuildOptions/BuildModel()) with no drawing at all -- lets
    // CMainFrameWindow::SyncRmlModel() bind the result into RmlUi instead of the legacy
    // TextList/RenderTipTextList path Render() still uses for its other callers
    // (WindowMuHelper.cpp, SiegeWarBase.cpp -- unrelated, still-legacy windows, unaffected by this
    // addition; Render() itself is unchanged apart from delegating to this).
