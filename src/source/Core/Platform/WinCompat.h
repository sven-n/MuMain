...
//
//   - On Windows: include <windows.h> and use native UTF-8 conversion.
//   - Elsewhere: provide the Win32 types and portable UTF-8 conversion.
//
// UTF-8/path helpers exist on both branches. Broader Win32 calls remain
// platform-specific; this header does not emulate the Win32 API.
#pragma once
...
inline void mu_wchar_to_utf8(const wchar_t* text, std::string& result)
{
    ...
    const int byteCount = ::WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, text, -1, nullptr, 0, nullptr, nullptr);
    if (byteCount <= 0)
    {
        return;
    }
    ...
    const int converted =
        ::WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, text, -1, result.data(), byteCount, nullptr, nullptr);
...
#ifndef RGB
#define RGB(r, g, b)                                                                                                   \
    (static_cast<COLORREF>((static_cast<BYTE>(r)) | (static_cast<WORD>(static_cast<BYTE>(g)) << 8) |                   \
