...
void RenderScene(HDC hDC)
{
    ...
    // DXP-15 increment 3: pairs with PlatformSwapBuffers()'s RHI::EndFrame() call --
    // reserved per-frame backend hook (currently a no-op on both backends, RHI_GL.cpp/
    // RHI_D3D11.cpp), called unconditionally here rather than gated on backend so any
    // future per-backend frame-start work doesn't need a second call site added later.
    RHI::BeginFrame();
