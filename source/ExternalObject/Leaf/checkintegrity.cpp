...
inline bool leaf::GenerateCheckSum(IN const void* pBuffer, size_t size, OUT LPCHECKSUMTABLE lpCheckSumTable) 
{ 
...
CCheckSumGenerator::CCheckSumGenerator()
{}
CCheckSumGenerator::~CCheckSumGenerator()
{ RemoveAllSegment(); }
...
bool CCheckSumGenerator::AddSegmentFromBuffer(const std::string& dataname, const void* pBuffer, size_t size)
{
	if(!GetSegmentInfo(dataname))
	{
		...
		if(GenerateCheckSumTable(pBuffer, size, &pSegmentInfo->cs_table))
		{
			...
			m_listSegment.push_back(pSegmentInfo);
			return true;
		}	
		delete pSegmentInfo;
	}
...
int CCheckSumGenerator::GetSegmentCount() const
